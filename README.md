...
- 这道题是涉及”数字信号处理“的Ti-MCU限定题：
  - 可在DMA配置中开启乒乓模式，使得采样点数为2048，以提高频率分辨率等等；
  - 其次可将时域补零至4096个点，提高计算分辨率等等；
  - MCU选型为C2000较有优势；
...
- 代码BUG（这个是我在电赛之后整理代码后发现的，故不改，留个纪念）
  ...
  - gameA.c中第152行代码，应该得把 char strBuf[6] 改为改 char strBuf[9]，实际中不改的话应该会溢出，导致程序跑飞才对（在“MSP432P4 Project\C_test\DrawWave”中可以测试），但在此工程中竟然是正常运行。

```c
/**   main.c 第132行代码 计算归一化幅值   **/
gyh[i] = ceil(fft_outputbuf[fx[i]] / fft_outputbuf[f0] * 100.0f) / 100.0f; // 向上取整
/* gameA.c 第152行代码 更新归一化幅值、THD */
