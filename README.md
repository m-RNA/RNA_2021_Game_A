...
​		感觉这个A题很像老师布置了检验MSP432学习成果的作业哈哈哈（不是），个人认为难点在于单品机片内ADC与DMA的联动配置，信号数字处理理论知识以及软件各算法的优化，如基波谐波的幅值查找、波形显示等的优化（个人见解哈哈哈）。  
...
- 定时器A2作为测量频率的输入捕获定时器（详情见 [6 外围电路](### 6 外围电路)）
...
```
> 使用两个小OLED的原因: 我们之前是做电源题的，考虑到降低功耗提高效率就使用了小OLED，怕OLED坏了，就买了很多备着，于是比赛时没有SPI协议或者IIC协议的较大屏幕，只能配合着OLED图形库，稍微添加了几条函数，使用IIC驱动两个小OLED。(其实有8080并口LCD，但是之前没备好驱动，感觉来不及移植了；其次是借到了一个UART串口屏，但我没学过，也感觉时间可能来不及）（队友也挺积极的，期间这个UART串口屏有一队友尝试学习被我阻止了，另一队友学习后并请教他人觉得不可行就没尝试了）(没错，单片机代码全由我一人敲写，这其实也合理，因为电源题一般一人负责软件，两人负责硬件)
...
注：过零比较器给定时器A2通道1，即P5.7捕获，以获信号的频率；不接过零比较器，默认以50x16kHz进行采样（此时测量50kHz的信号最准确，原因是算法中默认情况下，采集信号时，fs是根据f动态改变的，以获得最优的测量结果）
...
- 感谢积极细心的队友，硬件电路模块全部队友自制，而且在软件算法方面也给予了理论与实践帮助支持，在调试时帮忙揪出BUG，一起完成报告（还有包容我的小脾气）（说来惭愧，我们本来时做电源的，但电源题真是一言难尽，做仪器题实属半路出道；电赛前两天队友叫我学习一下FFT跟THD，老实的我学习并实践了，因此出题的前一晚我们就已经完成了基础部分）（队友神预判）
- 感谢B站UP主“物联世界”视频教程；
...
- 感谢爸妈，支持我烧钱打电赛；感谢女朋友，一直没出现，让我专心完成电赛；
