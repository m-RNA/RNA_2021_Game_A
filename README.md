...
- 这道题是涉及“数字信号处理”的Ti-MCU限定题：
  - 可将时域补零至4096个点，以提高计算分辨率等；
  - 其次可在配置DMA时开启乒乓模式，使得采样点数为2048，以提高频率分辨率等；
  - MCU选型为C2000较有优势；
- 可以用滞回比较器来解决过零比较器的多零点问题，来减轻软件压力；
- 可以免去按键，改为开机自动测量，并多次测量，将测量结果去掉最值后求平均值，将单片机OLED的测量动画与手机显示动画去除，实时显示测量结果；
- 代码BUG（这个是我在电赛之后整理代码后发现的，故不改，留个纪念）
  - main.c 中第132行代码，应该得把向上取整 ceil() 改为向下取整 floor() 比较合适，这样能减少归一化幅值的误差；
  - gameA.c中第152行代码，应该得把 char strBuf[6] 改为 char strBuf[9]，实际中不改的话应该会溢出，导致程序跑飞才对（在“MSP432P4 Project\C_test\DrawWave”中可以测试），但在此工程中竟然能正常运行🤪

```c
// main.c 第132行代码 
gyh[i] = ceil(fft_outputbuf[fx[i]] / fft_outputbuf[f0] * 100.0f) / 100.0f; 

// gameA.c 第152行代码 
char strBuf[6]; // !! Waring  应该改strBuf[9]
```
